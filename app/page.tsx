...
function DepthScrollStory() {
  const trackRef = useRef<HTMLElement | null>(null);
  const [progress, setProgress] = useState(0);
  useEffect(() => { const update = () => { const track = trackRef.current; if (!track) return; const distance = Math.max(1, track.offsetHeight - window.innerHeight); const next = Math.min(1, Math.max(0, -track.getBoundingClientRect().top / distance)); setProgress((current) => Math.abs(current - next) > 0.003 ? next : current); }; update(); window.addEventListener("scroll", update, { passive: true }); window.addEventListener("resize", update); return () => { window.removeEventListener("scroll", update); window.removeEventListener("resize", update); }; }, []);
  const phase = progress < .25 ? "enter" : progress < .5 ? "story" : progress < .75 ? "details" : "schedule";
  const stageStyle = { "--depth-progress": progress } as CSSProperties;
  return <section className="depth-scroll-track reveal is-visible" id="story" ref={trackRef} style={stageStyle}><div className="depth-sticky-stage"><div className={`depth-stage depth-phase-${phase}`}><div className="depth-stage-grid" /><div className="depth-stage-label"><span>SCROLL / {String(Math.round(progress * 100)).padStart(2, "0")}</span><span>FRAME {phase === "enter" ? "01" : phase === "story" ? "02" : phase === "details" ? "03" : "04"} / 04</span></div><div className="depth-frame depth-frame-back"><span>THE<br />BEGINNING</span></div><div className="depth-frame depth-frame-mid"><img src="/assets/images/bloom-couple.webp" alt="A couple celebrating together" /><span>THE GLASSHOUSE / 2026</span></div><div className="depth-frame depth-frame-front"><div className="depth-frame-inner"><span>NR / 17.10.26</span><strong>Nara<br /><i>& Raka</i></strong><small>Somewhere between the first hello<br />and the forever after.</small></div></div><div className="depth-scene-copy"><div className="depth-scene depth-scene-enter"><p className="eyebrow">A modern invitation</p><h2>Start<br /><i>here.</i></h2><span>Scroll to open the first frame</span></div><div className="depth-scene depth-scene-story"><p className="eyebrow">The story</p><h2>One frame<br /><i>at a time.</i></h2><span>Five years, one very good idea</span></div><div className="depth-scene depth-scene-details"><p className="eyebrow">The details</p><h2>Make room<br /><i>for this.</i></h2><span>Ubud · Saturday · 16:00</span></div><div className="depth-scene depth-scene-schedule"><p className="eyebrow">The programme</p><h2>Stay for<br /><i>the whole thing.</i></h2><span>Vows · dinner · dancing</span></div></div></div></div></section>;
}

function DepthBody() {
  return <div className="grand-body grand-body-depth"><DepthScrollStory /><section className="depth-details reveal" id="details"><div><p className="eyebrow">After the opening frame</p><h2>Everything<br /><i>falls into place.</i></h2></div><div className="depth-detail-grid"><article><span>01 / ARRIVE</span><strong>15:30</strong><p>Find the room, find your people, and settle into the afternoon.</p></article><article><span>02 / PROMISE</span><strong>16:00</strong><p>The ceremony begins when the light is exactly right.</p></article><article><span>03 / STAY</span><strong>18:30</strong><p>Dinner, dancing, and one more song before the night ends.</p></article></div></section><section className="depth-closer reveal" id="schedule"><div className="depth-closer-mark">12<span>—</span>26</div><div><p className="eyebrow">The last frame</p><h2>See you<br /><i>inside.</i></h2><p>Keep a little room in your calendar. We will keep the lights on.</p></div></section></div>;
}
